...
@weighted_loss
def inner_ciou(pred: Tensor, target: Tensor, ratio, eps: float = 1e-7) -> Tensor:
    ...
    # Inner IOU
    (x1, y1, w1, h1), (x2, y2, w2, h2) = pred.chunk(4, -1), target.chunk(4, -1)
    ...
    cious = inner_iou - (rho2 / c2 + v * alpha)
    loss = 1 - cious.clamp(min=-1.0, max=1.0)
    return loss
    
@weighted_loss
def inner_siou(pred, target, ratio = 1.0, xywh=True, eps=1e-7): 
    box1 = pred
    box2 = target
