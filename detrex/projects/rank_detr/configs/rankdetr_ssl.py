...
from .models.rank_detr_master import model
...
#def my_factor_func 
optimizer.params.lr_factor_func = partial(get_vit_lr_decay_rate, lr_decay_rate=0.85, num_layers=24)
#optimizer.params.lr_factor_func = lambda module_name: 0.1 if ('backbone' in module_name) else 1
#optimizer.params.lr_factor_func = lambda module_name: 0.5 if (('backbone' in module_name) or ('dist_token' in module_name) or ('pos_embed' in module_name)) else 1
...
optimizer.params.overrides = {#'backbone.backbone': {'lr': 1e-5, 'weight_decay': 0.01},
                              #'backbone.norm': {'lr': 1e-5, 'weight_decay': 0.},
                              #'backbone.blocks': {'lr': 1e-5, 'weight_decay': 0.01},
                                    #'backbone.norm': {'lr_mult':0.1, 'lr_factor' : 0.1, 'weight_decay' : 0.01},
                                    #'backbone.backbone.blocks': {'weight_decay' : 0.01},
                                    'pos_embed': {'wd_mult' : 0, 'weight_decay' : 0},
                                    'dist_token': {'wd_mult' : 0, 'weight_decay' : 0},
}
