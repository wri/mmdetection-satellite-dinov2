...
class SSLVisionTransformer(DinoVisionTransformer):
    ...
    def __init__(self,
        ...
        print(type(out_indices), out_indices)
        print(type(final_norm), final_norm)
        print(type(frozen_stages), frozen_stages)
        ...
        print(out_indices, type(out_indices))
        ...
        #if pretrained:
        #print("TURN THIS OFF IF RESUMING!!")
    ...
    def init_weights(self, pretrained = '/home/ubuntu/mmdetection/models/SSLhuge_satellite.pth'):
        print("THE WEIGHT INIT IS CALLED")
        logger.info("WEWERSARASDFSFSD")
        print("init_weights", pretrained)
        self.init_cfg = dict(self.init_cfg)
        counter = 0
        '''
        rand_weights = []
        for module_name, module in self.named_modules():
            ...
            for module_param_name, value in module.named_parameters(recurse=False):
                ...
                counter += 1
                if counter < 1000: #module_name, module_param_name, 
                    rand_weights.append(value.detach().cpu().numpy().flatten()[:5])
        '''        
        #print(isinstance(self.init_cfg, dict)
        if (isinstance(self.init_cfg, dict)):# and self.init_cfg.get('type') == 'Pretrained'):
            print("Initializing backbone from /home/ubuntu/mmdetection/models/SSLLarge.pth")
            checkpoint = torch.load('/home/ubuntu/mmdetection/models/SSLLarge.pth', map_location = torch.device("cuda"))
            ...
            print("MOOORE PRRIIINT HEREEE")
            #print(self.patch_embed.proj.weight.detach().cpu().numpy().flatten()[:5])
            #counter = 0
            #loaded_weights = []
            #for module_name, module in self.named_modules():
            #if 'block' in module_name:
                #for module_param_name, value in module.named_parameters(recurse=False):
                    ...
                    #counter += 1
                    #if counter < 1000:
                        #loaded_weights.append(value.detach().cpu().numpy().flatten()[:5])
            #for r, l in zip(rand_weights, loaded_weights):
            #    print(r - l)
        else:
    ...
    def forward(self, x):
        ...
        with torch.set_grad_enabled(not self.detach):
            ...
            #    outs[i] = ops[i](outs[i])
                #$print(outs[i].shape)
            outputs = {}
            for name, stage in zip(['p2', 'p3', 'p4', 'p5'], outs):
                outputs[name] = stage#.permute(0, 3, 1, 2)
            return outputs
