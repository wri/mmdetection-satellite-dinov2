...
# python tools/train_net.py --config-file projects/stabledino/configs/stabledino_swinL_4scale_12ep.py     --num-gpus 1     dataloader.train.total_batch_size=2     train.output_dir="./output/stabledino_swin2"     train.test_with_nms=0.2

model = L(DINO)(
    backbone=MMDetBackbone(backbone=SSLVisionTransformer(
        img_size=(512, 512),
        patch_size=16,
        embed_dim=1024,
        depth=24,
        ...
        frozen_stages = -1,
        qkv_bias=True,
        init_cfg=dict(type='Pretrained', checkpoint='/home/ubuntu/mmdetection/models/SSLLarge.pth'),
        out_indices=[4, 11, 17, 23],
    ),
    output_shapes = [ShapeSpec(channels=256), #?
                     ShapeSpec(channels=512),
    ...
    neck=L(ChannelMapper)(
        input_shapes={
            ...
            "p4": ShapeSpec(channels=512),
            "p5": ShapeSpec(channels=1024),
            "p6": ShapeSpec(channels=1024),
        },
